...
// The `params` object contains the dynamic segments of the URL, allowing you to access the `postid` parameter in this case.
// For example, if the URL is `/api/posts/123`, `params.postid` will be `123`.
// The `params` object is automatically populated by Next.js based on the dynamic segments in the URL. In this case, it will contain the `postid` parameter.
export async function GET(request, { params }){
  ...
  return NextResponse.json({ message: `Searching post ${params.postid} with search params: ${searchParams}` });
}
...
export async function POST(request){
  ...
  return NextResponse.json({ message: `Creating post with data: ${data}` });
}
